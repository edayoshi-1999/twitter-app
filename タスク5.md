...
**インデックス**:
- `follower_id` （特定ユーザーがフォローしている人一覧）
...
| `likes` | `user_id` | 特定ユーザーがいいねしたツイート一覧 |
| `follows` | `follower_id` | 特定ユーザーがフォローしている人一覧 |
...
return new class extends Migration
{
    public function up(): void
    {
        Schema::create('likes', function (Blueprint $table) {
            ...
            $table->index('user_id');   // ユーザーのいいね一覧用
        });
    }
...
3. ✅ インデックス: `tweet_id` と `user_id` で高速検索
...
return new class extends Migration
{
    public function up(): void
    {
        Schema::create('follows', function (Blueprint $table) {
            ...
            // インデックス: パフォーマンス最適化
            $table->index('follower_id');   // フォロー中一覧用
